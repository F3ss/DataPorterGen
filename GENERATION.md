...
Поэтому пустой `fields: {}` означает «скопировать шаблон без заданных преобразований», но `_id` всё равно должен быть безопасно разрешён. Источник и цель могут совпадать: snapshot-префиксы всех коллекций завершаются до первой записи, поэтому документы, записанные текущим запуском, не становятся новыми шаблонами.
